...
class Judger:
    ...
    def __init__(self, accusation_path, law_path):
        ...
        f = open(accusation_path, "rb")
        self.task1_cnt = 0
        for line in f:
            self.task1_cnt += 1
            self.accu_dic[line[:-1].decode('utf8')] = self.task1_cnt
    ...
    def gen_new_result2(self, result, truth, label):
        ...
        sc = 0
        imp1 = label["imprisonment"]
        imp2 = truth["imprisonment"]
    ...
    def gen_score(self, arr):
        ...
        return f_, sumf*1.0/len(arr), (f_ + sumf * 1.0 / len(arr)) / 2.0
    ...
    def test2(self, truthes, user_predicts):
        cnt = 0
        result = [[], [], {}]
        for a in range(0, self.task1_cnt):
            result[0].append({"TP": 0, "FP": 0, "TN": 0, "FN": 0})
...
if __name__ == "__main__":
    ...
    rst = judge.test(sys.argv[1], sys.argv[2])
    result = judge.get_score(rst)
    print(result)
