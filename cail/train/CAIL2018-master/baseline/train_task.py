...
def read_testData(filename):
    ...
    train_num = read_trainData(filename, train_data, accu_label, law_label, time_label, train_labels, train_predict_labels)

    return train_data, train_labels, train_predict_labels
...
if __name__ == '__main__':
    ...
    print('reading train data.')
    train_num = read_trainData(train_fname, train_data, accu_label, law_label, time_label, train_labels, train_predict_labels)
    ...
    #for SVC
    SVC_parameters = {'tfidf__max_df': [0.75, 1.],   #过滤了几十个词
    ...
    def pipe_train(param):
        param_str = json.dumps(param)
        clf = LinearSVC(**params['clf'])
        clf_name = 'LinearSVC'
        pipeline = Pipeline([
        ...
        def predict_rst(data, label):
            _predicts = pipeline.predict(test_data)
        ...
        train_rst, train_rst = predict_rst(train_data, train_labels) 
        test_rst,test_rstr = predict_rst(test_data, test_labels)

        sinfo = 'TrainFile: %s\tSeg: %s\tModel: %s\tParam: %s\tTrain_Result: %s\tTest_Result: \t%s'\
                % (train_fname, seg_method, clf_name, param_str, train_rst, test_rstr)
        logger.info(sinfo)
